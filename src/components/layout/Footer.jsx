...
/* ══════════════════════════════════════════
   REAL SOCIAL LINKS — all 6 platforms
══════════════════════════════════════════ */
const SOCIALS = [
...
const Magnetic = ({ children, strength = 0.3 }) => {
  ...
  return (
      <motion.div ref={ref} style={{ x: sx, y: sy }} onMouseMove={onMove} onMouseLeave={() => { x.set(0); y.set(0); }}>
        {children}
      </motion.div>
  );
};
...
const FooterNavLink = ({ link, index }) => {
  ...
  const inView = useInView(ref, { once: false });
  return (
      <motion.li ref={ref} initial={{ opacity: 0, x: -22 }} animate={inView ? { opacity: 1, x: 0 } : { opacity: 0, x: -22 }} transition={{ delay: index * 0.06, type: "spring", stiffness: 220 }}>
        <motion.a href={`#${link.id}`} className="flex items-center gap-2 text-sm font-semibold"
                  onHoverStart={() => setHovered(true)} onHoverEnd={() => setHovered(false)}
                  animate={{ color: hovered ? "#8DFF69" : "rgba(107,114,128,1)" }} whileHover={{ x: 7 }} transition={{ duration: 0.18 }}
        >
          <motion.span className="w-1.5 h-1.5 rounded-full"
                       animate={{ background: hovered ? "#8DFF69" : "rgba(107,114,128,0.4)", scale: hovered ? 1.6 : 1, boxShadow: hovered ? "0 0 7px #8DFF69" : "none" }} transition={{ duration: 0.18 }} />
          {link.label}
        ...
        {visible && (
            <Magnetic strength={0.55}>
              <motion.button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
                             ...
                             style={{ background: hovered ? "linear-gradient(135deg,#8DFF69,#61DAFB)" : "rgba(141,255,105,0.08)", border: "1px solid rgba(141,255,105,0.28)" }}
                             initial={{ opacity: 0, scale: 0, rotate: -90 }} animate={{ opacity: 1, scale: 1, rotate: 0 }} exit={{ opacity: 0, scale: 0, rotate: 90 }}
                             whileHover={{ scale: 1.12, boxShadow: "0 0 26px rgba(141,255,105,0.42)" }} whileTap={{ scale: 0.88 }}
              ...
              >
                ...
                <motion.div animate={{ y: hovered ? -2 : 0 }} transition={{ type: "spring", stiffness: 500, damping: 8, repeat: hovered ? Infinity : 0, repeatType: "reverse" }}>
  ...
  return (
      <div ref={ref} className="relative my-12 h-[1px] overflow-visible">
        <motion.div className="absolute inset-0"
                    ...
                    initial={{ scaleX: 0, opacity: 0 }} animate={inView ? { scaleX: 1, opacity: 1 } : { scaleX: 0, opacity: 0 }} transition={{ duration: 1.1, ease: "easeOut" }} />
        <motion.div className="absolute top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full"
                    ...
                    animate={{ left: ["0%", "100%", "0%"] }} transition={{ duration: 5.5, repeat: Infinity, ease: "easeInOut" }} />
      </div>
  );
};
...
const FooterLogo = () => {
  ...
  return (
      ...
      >
        <motion.div className="relative w-11 h-11 rounded-2xl flex items-center justify-center overflow-hidden"
                    animate={{ background: hovered ? "rgba(141,255,105,0.2)" : "rgba(141,255,105,0.07)", boxShadow: hovered ? "0 0 22px rgba(141,255,105,0.4),inset 0 0 18px rgba(141,255,105,0.1)" : "none" }}
                    style={{ border: "1px solid rgba(141,255,105,0.18)" }} transition={{ duration: 0.28 }}
        >
          <AnimatePresence>
            {hovered && <motion.div className="absolute inset-0 rounded-2xl bg-[#8DFF69]/10" initial={{ scale: 0 }} animate={{ scale: 2.2, opacity: 0 }} transition={{ duration: 0.45 }} />}
          </AnimatePresence>
        ...
        <div className="flex items-center">
          {"HASIB".split("").map((l, i) => (
              <motion.span key={i} className="font-black text-xl tracking-tighter"
                           ...
                           transition={{ delay: i * 0.035, type: "spring", stiffness: 420 }}>{l}</motion.span>
          ))}
          <motion.span className="text-lg ml-1" animate={{ rotate: hovered ? 22 : 0 }} transition={{ type: "spring", stiffness: 280 }}>😊</motion.span>
        </div>
      </motion.div>
  );
};
...
const Footer = () => {
  ...
  const cV = { hidden: {}, visible: { transition: { staggerChildren: 0.09, delayChildren: 0.04 } }, exit: { transition: { staggerChildren: 0.04, staggerDirection: -1 } } };
  const iV = {
  ...
  return (
      <footer ref={footerRef} className="relative overflow-hidden pt-20 pb-10"
              style={{ background: "#030712", borderTop: "1px solid rgba(255,255,255,0.04)" }}
      >
        <style>{`@keyframes gridSlide{0%{background-position:0 0}100%{background-position:60px 60px}}`}</style>
        ...
        <div className="absolute inset-0 pointer-events-none" style={{
          ...
          backgroundSize: "60px 60px", animation: "gridSlide 5s linear infinite",
        }} />

        {/* Blobs */}
        <motion.div className="absolute top-0 left-0 w-[480px] h-[480px] rounded-full pointer-events-none"
                    style={{ background: "radial-gradient(circle,rgba(141,255,105,0.055) 0%,transparent 70%)", y: y1 }} />
        <motion.div className="absolute bottom-0 right-0 w-[380px] h-[380px] rounded-full pointer-events-none"
                    style={{ background: "radial-gradient(circle,rgba(97,218,251,0.045) 0%,transparent 70%)", y: y2 }} />

        <div className="container mx-auto px-4 relative z-10 max-w-6xl">
          <AnimatePresence mode="wait">
            {inView && (
                <motion.div key="main" variants={cV} initial="hidden" animate="visible" exit="exit"
                ...
                >
                  ...
                  <motion.div variants={iV} className="space-y-6">
                    ...
                    <div className="space-y-3">
                      <motion.a href={`mailto:${PERSONAL_INFO.email}`} className="flex items-center gap-3 w-fit" whileHover={{ x: 4 }}>
                        <motion.div className="p-2 rounded-xl" style={{ background: "rgba(141,255,105,0.07)", border: "1px solid rgba(141,255,105,0.14)" }}
                                    whileHover={{ background: "rgba(141,255,105,0.14)", boxShadow: "0 0 14px rgba(141,255,105,0.18)" }}>
                          <Mail className="w-4 h-4 text-[#8DFF69]" />
                        </motion.div>
                        <motion.span className="text-gray-500 text-sm font-medium" whileHover={{ color: "#8DFF69" }}>{PERSONAL_INFO.email}</motion.span>
                      </motion.a>
                  ...
                  <motion.div variants={iV} className="flex flex-col md:items-center">
                    <div className="w-fit">
                      <p className="font-black mb-7 text-[10px] uppercase tracking-[0.22em]" style={{ color: "rgba(255,255,255,0.4)" }}>Quick Links</p>
                      <ul className="space-y-4">{NAV_LINKS.map((l, i) => <FooterNavLink key={l.id} link={l} index={i} />)}</ul>
                    </div>
                  </motion.div>
                  ...
                  <motion.div variants={iV} className="flex flex-col md:items-end gap-7">
                    <div className="w-full">
                      <p className="font-black mb-7 text-[10px] uppercase tracking-[0.22em] md:text-right" style={{ color: "rgba(255,255,255,0.4)" }}>Social Connect</p>
                      <div className="flex gap-3 flex-wrap md:justify-end">
                        {SOCIALS.map((s, i) => <SocialIcon key={s.key} {...s} index={i} />)}
                      </div>
                    </div>
                    {/* Clock */}
                    <motion.div className="flex items-center gap-3 px-4 py-2.5 rounded-2xl"
                                style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}
                                whileHover={{ borderColor: "rgba(141,255,105,0.2)", background: "rgba(141,255,105,0.02)" }} transition={{ duration: 0.28 }}
                    >
          ...
          <AnimatePresence mode="wait">
            {inView && (
                <motion.div key="bottom" className="flex flex-col md:flex-row justify-between items-center gap-5"
                            initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ delay: 0.35, duration: 0.55 }}
                >
                  <motion.div className="text-gray-600 text-xs font-medium tracking-wide" whileHover={{ color: "#9ca3af" }}>
                    © {new Date().getFullYear()}{" "}
                    <motion.span className="text-gray-400 font-bold" whileHover={{ color: "#8DFF69" }}>{PERSONAL_INFO.name}</motion.span>. All Rights Reserved.
                  </motion.div>
                  <motion.div className="flex items-center gap-2 text-xs text-gray-600">
                    ...
                    <span>using</span>
                    <motion.span className="font-black px-2 py-0.5 rounded-lg text-[10px] uppercase tracking-wider inline-flex items-center gap-1"
                    ...
                    ><Code2 className="w-3 h-3" />React & Tailwind</motion.span>
                  </motion.div>
